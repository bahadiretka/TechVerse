...
class PaymentHandler: NSObject {
    ...
    func shippingMethodCalculator() -> [PKShippingMethod] {
        ...
        if let shippingEnd = shippingEnd, let shippingStart = shippingStart {
            ...
            shippingDelivery.detail = "TechVerse sent to your address"
    ...
    func startPayment(products: [CartItem], total: Int, completion: @escaping PaymentCompletionHandler) {
        completionHandler = completion

        paymentSummaryItems = []

        products.forEach { product in
            let item = PKPaymentSummaryItem(label: product.product.name, amount: NSDecimalNumber(string: "\(product.product.price).00"), type: .final)
        ...
        paymentRequest.merchantIdentifier = "techverseshop"
